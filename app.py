...
# --- CPT Codes (truncated for demo — include full list in real app) ---
cpt_codes = [
    ("90832", "Outpatient 15-30 minutes"),
    ("90834", "Outpatient"),
    ("90837", "OP Session 61-90 Minutes"),
]
    ...
    if ctx.audio_receiver:
        ...
        if audio_bytes:
            try:
                ...
                st.warning(f"Transcription failed: {str(e)}")
...
if session_input:
    ...
    if st.button("Generate Summary"):
        ...
        formats = [fmt for fmt, enabled in zip(["SOAP", "DAP"], [generate_soap, generate_dap]) if enabled]
        ...
        for fmt in formats:
            ...
            with st.spinner(f"Generating {fmt} note..."):
                try:
                    ...
                    # --- CPT Suggestion ---
                    suggested_cpt = next(
                        (f"{code} – {desc}" for code, desc in cpt_codes if re.search(rf"\\b{code}\\b", summary)),
                        None
                    )
                    ...
                    # --- CPT Manual Selection ---
                    st.markdown("### 📝 Confirm or override CPT code")
                    options = [f"{code} – {desc}" for code, desc in cpt_codes]
                    default_index = options.index(suggested_cpt) if suggested_cpt in options else 0
                    selected_cpt = st.selectbox("CPT Code", options, index=default_index)
    ...
    st.info("Provide notes using your selected input method above.")
